...
mod tests {
    ...
    #[test]
    fn send_json_request_id_zero_returns_structured_error_before_enqueue() {
        let sink = Arc::new(CollectSink::new());
        let rt = Runtime::new(sink.clone());
    ...
    #[test]
    fn send_json_empty_method_returns_structured_error_before_enqueue() {
        let sink = Arc::new(CollectSink::new());
        let rt = Runtime::new(sink.clone());
    ...
    fn host_completion_rejects_zero_operation_id() {
        ...
        match &sink.wait_len(1)[0] {
            Event::Error {
                ...
                assert_eq!(error.code, ErrorCode::InvalidParams);
                assert_eq!(error.details["operationId"], 0);
            }
            other => panic!("expected zero operationId error, got {other:?}"),
        }
    ...
    #[test]
    fn runtime_cancel_command_rejects_zero_target_request_id() {
        let sink = Arc::new(CollectSink::new());
        let rt = Runtime::new(sink.clone());
        rt.send_json(
            include_str!(
        ...
        match &sink.wait_len(1)[0] {
            ...
            other => panic!("expected zero cancel target invalid params, got {other:?}"),
        }
    }
    ...
    #[test]
    fn remote_host_http_completion_rejects_invalid_status_shape() {
        let sink = Arc::new(CollectSink::new());
        let rt = Runtime::new(sink.clone());
        rt.send(Command::new(
            26,
