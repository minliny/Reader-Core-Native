...
public final class ReaderCoreRuntime implements Closeable {
    ...
    public ReaderCoreRuntime(String configJson) {
        ...
        if (created == 0) {
            throw new ReaderCoreException("rc_runtime_create failed: " + lastErrorMessage());
        }
        this.handle = created;
    }
    ...
    public synchronized void send(String commandJson) {
        ...
        if (status != 0) {
            throw new ReaderCoreException(
                    status,
                    "rc_runtime_send failed with status " + status + ": " + lastErrorMessage());
        }
    }
    ...
    public synchronized int cancel(long requestId) {
        ...
        if (status != 0) {
            throw new ReaderCoreException(
                    status,
                    "rc_runtime_cancel failed with status " + status + ": " + lastErrorMessage());
        }
        return status;
    }
    ...
    private static String lastErrorMessage() {
        String message = NativeCoreBridge.lastError();
        return message.isEmpty() ? "no structured error" : message;
    }
