...
fn set_mode(slot: &mut Option<Mode>, mode: Mode) -> Result<(), CoreError> {
    if slot.is_some() {
        return Err(CoreError::invalid_message(
            "only one of --info, --ping, --host-smoke, --json, --stdin, or --fixture-vertical may be used",
        ));
    }
    *slot = Some(mode);
    Ok(())
...
fn print_usage() {
    eprintln!(
        "usage: reader-cli [--info|--ping|--host-smoke|--json '<command>'|--stdin|--fixture-vertical <path>] [--config-json '<config>']"
    );
}
