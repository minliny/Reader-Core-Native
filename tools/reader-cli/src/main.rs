...
fn set_mode(slot: &mut Option<Mode>, mode: Mode) -> Result<(), CoreError> {
    if slot.is_some() {
        return Err(CoreError::invalid_message(
            "only one of --info, --ping, --status, --host-smoke, --conformance, --json, --stdin, --fixture-vertical, --booksource-fixture, --host-replay, or --host-replay-suite may be used",
        ));
    }
    *slot = Some(mode);
    Ok(())
...
fn print_usage() {
    eprintln!(
        "usage: reader-cli [--info|--ping|--status|--host-smoke|--conformance|--json '<command>'|--stdin|--fixture-vertical <path>|--booksource-fixture <path>|--host-replay <path>|--host-replay-suite <path>] [--config-json '<config>']"
    );
}
