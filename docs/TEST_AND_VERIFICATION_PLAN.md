...
```bash
# 批量测试（离线回放模式，用录制的响应）
cargo run -p reader-cli -- --test-corpus-offline \
...
# 批量测试（真实网络模式，可选）
cargo run -p reader-cli -- --test-corpus \
  tests/fixtures/corpus/corpus-manifest.json \
  --keyword "斗破苍穹" \
  --out reports/tooling/corpus-batch-live-result.json \
  --timeout 15 \
  [--max-sources 10] \
  [--record tests/fixtures/corpus/recorded/]
```
...
```bash
# 对 P0 源发起真实搜索请求（live 冒烟 = --test-corpus + --max-sources 10 + --priority P0）
cargo run -p reader-cli -- --test-corpus \
  tests/fixtures/corpus/corpus-manifest.json \
  --keyword "斗破苍穹" \
...
2. 四端各跑一遍：
   - CLI: reader-cli --test-corpus-offline（离线回放）
   - Android: connectedAndroidTest --test-corpus-offline（MockWebServer 回放）
   - iOS: xctest --test-corpus-offline（URLProtocol mock 回放）
   - HarmonyOS: hdc test --test-corpus-offline（OHOS HTTP mock 回放）
...
release-gate:
  ...
  steps:
    ...
    - # 四端 corpus benchmark
    - # CLI
    - cargo run -p reader-cli -- --test-corpus-offline tests/fixtures/corpus/corpus-manifest.json --out reports/tooling/corpus-benchmark/cli/
...
4. **新增 reader-cli `--test-corpus-offline` 子命令** — 离线回放批量测试
...
13. **新增 reader-cli `--test-corpus`** — 真实网络冒烟（live，含 --record 录像）
...
- reader-cli `--test-corpus-offline` — 离线回放批量测试子命令
- reader-cli `--test-corpus` — 真实网络批量/冒烟子命令（含 --record 录像）
