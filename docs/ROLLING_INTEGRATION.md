...
`codex/core-product-integration` currently includes
`origin/codex/remote-reading-vertical` on top of the foundation baseline. The
product lane is complete for Core-side smoke:
...
- Content/storage/progress evidence is fixture/inline-response only; storage is
  V1 in-memory cache/progress, not a durable platform database.
...
- Do not treat iOS Swift typecheck as runtime proof. Runtime smoke must exercise
  the wrapper against the Core ABI at execution time.
...
- iOS wrapper runtime smoke: integrate only after wrapper execution evidence,
  not just Swift typecheck, into `codex/ios-integration`.
