...
| Legado BookSource compat | `codex/booksource-compat-protocol` | Legado BookSource raw object、unknown field、raw rule 保真和 `source.import` conformance | domain/contract/runtime/CLI tests + conformance |
| Legado DSL executor | `codex/legado-rule-dsl-executor` | 独立实现 Legado CSS 管道链 DSL，不通过 `RuleStepSpec` 硬凑 | rule/content tests + CLI fixture |
| Rust product core | `codex/rust-product-core-integration` | JS、request、reading、storage、local、RSS、sync | crate tests + CLI fixture |
...
3. Legado DSL executor 最小 search/detail/toc/content 闭环。
4. JS/request/reading 核心能力。
5. Storage/local/RSS/sync/TTS 契约。
6. iOS/Android/HarmonyOS wrapper 与 host adapter。
7. CLI + 三端 benchmark。
8. 退役旧业务核心路径。
