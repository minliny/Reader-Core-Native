...
> 注 3：按"严格达标 = 真实 Legado 书源跑通（🟢）"口径，达标率 = 0%（无 🟢 项）；
> 按"证据级别 🟢+🔵 = 已实现"口径，达标率 = 36%（39/109）。

### 诚实评估

- **Core 侧已实现（🟢+🔵）: 39/109 = 36%**（含 MultiRule blocker 已解、Bookmark/BookGroup/ReadRecord dispatch 已 WIRED、RSS 源解析有合成 fixture 测试）
...
> 达标率口径说明：按"严格达标 = 真实 Legado 书源跑通（🟢）"口径，达标率 = 0%（无 🟢 项）；
> 按"证据级别 🟢+🔵 = 已实现"口径，达标率 = 36%（39/109）。
> 早期文档引用的"~31%/32%"为按 Status ✅ 计数的旧口径（34/109 ≈ 31%），本文统一改为按证据级别标记计数。
