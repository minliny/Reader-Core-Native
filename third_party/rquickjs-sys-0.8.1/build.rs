...
fn apple_ios_bindgen_cflags(target: &str) -> Vec<String> {
    let (sdk, deployment_flag) = match target {
        "aarch64-apple-ios" => ("iphoneos", "-miphoneos-version-min=13.0"),
        "aarch64-apple-ios-sim" => ("iphonesimulator", "-mios-simulator-version-min=13.0"),
        _ => return Vec::new(),
    };
    ...
    let output = process::Command::new("xcrun")
        ...
        .output()
        .unwrap_or_else(|error| panic!("failed to invoke xcrun for {sdk}: {error}"));
    if !output.status.success() {
        panic!(
    ...
    vec!["-isysroot".into(), sysroot, deployment_flag.into()]
}
...
fn main() {
    ...
    bindgen_cflags.extend(apple_ios_bindgen_cflags(&target));
    ...
    V: AsRef<str> + 'a,
{
    ...
    let mut cflags = vec![format!("--target={}", bindgen_clang_target(&target))];
