...
OHOS、Android、iOS 平台产物脚本按 [ARCHITECTURE.md](./ARCHITECTURE.md)
阶段 1/2 补齐。当前 `build-harmony-napi.sh` 验证 Rust staticlib 能链接为
HarmonyOS NAPI `.so`；`build-android-jni.sh` 验证 Rust staticlib 能链接为
Android JNI `.so`；App 仓库侧加载和真机集成仍需分别完成。
